...
export function Sidebar({ activeTab, onTabChange, user, onLogout }: SidebarProps) {
  const email = user?.email || 'A carregar...';
  ...
  return (
    <aside className="w-64 bg-white shadow-sidebar h-full flex flex-col z-20">
      ...
      <nav className="flex-1 px-4 flex flex-col gap-2">
        {navItems.map((item) => (
          <div key={item.id} className={`nav-item px-4 py-3 rounded-xl flex items-center gap-3 ${activeTab === item.id ? 'active bg-brand-lavender text-brand-blue font-semibold' : ''}`} onClick={() => onTabChange(item.id)}>
      ...
      <div className="p-6 mt-auto flex flex-col gap-3">
        <div className="bg-gray-50 rounded-2xl p-4 border border-gray-100 flex items-center gap-3 overflow-hidden">
          <img src={`https://ui-avatars.com/api/?name=${encodeURIComponent(email)}&background=378ADD&color=fff`} alt="Profile" className="w-10 h-10 rounded-full shadow-sm shrink-0" />
